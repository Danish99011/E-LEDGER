...
public class EmployeemanagerApplication {
    ...
    //corsconfiguration and cors(CROSS-ORIGIN RESOURCE SHARING) filter are used for CORS problem which means the backend is in 8080 port and frontend in 4200 so backend doesn't allow frontend to access and share data hence we need to do this
    @Bean
    public CorsFilter corsFilter() {
        ...
        corsConfiguration.setExposedHeaders(Arrays.asList("Origin","Access-Control-Allow-Origin","Content-Type","Accept",
                "Authorization","Access-Control-Request-Method","Access-Control-Request-Credentials"));
